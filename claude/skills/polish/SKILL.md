...
You are orchestrating the autobots post-implementation pipeline. This pipeline
...
1. Spawn the `autobot-simplifier` sub-agent using the Task tool:
   - **subagent_type**: `autobot-simplifier`
   - **prompt**: Include:
...
1. Spawn the `autobot-reviewer` sub-agent using the Task tool:
   - **subagent_type**: `autobot-reviewer`
   - **prompt**: Include:
...
[actionable suggestions for improving the autobot's prompts/skills/config]
